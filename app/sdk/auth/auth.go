...
func (a *Auth) GenerateToken(claims Claims) (string, error) {
	// Fetch an active kid from the key lookup.
	kid, err := a.keyLookup.(interface {
		ActiveKID() (string, error)
	}).ActiveKID()
	if err != nil {
		return "", fmt.Errorf("no active kid available: %w", err)
	}

	// Create the token with the claims and add the kid to the header.
